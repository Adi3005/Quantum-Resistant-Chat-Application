from chat import poly

class NTRU: #Class for NTRUEncrypt defined
    def __init__(self, N, p, q, f, g):
        ...
        print("==== Generating public and private keys ====") #For key generation, parameters (N,p,q) are defined
        print("Values used:")
        print(" N=", N)
        print(" p=", p)
        print(" q=", q)
        print("========")
        print("\nSender picks two polynomials (g and f):")
        print(" f(x)=", f)
        print(" g(x)=", g)
...
def decrypt(encmsg, f, f_p, p, q, D, l):
    ...
    for e in encmsg:
        ...
        print("Decrypted message block:\t", tmp[:l[j]])
